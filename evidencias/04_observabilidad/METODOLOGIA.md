...
**Qué demuestra:** Loki está operativo y listo para recibir logs.
...
**Qué debe aparecer:** `ready`

---

## EV-15 — `ev15_promtail_targets.png`

**Qué demuestra:** Promtail tiene los tres jobs activos y en estado `up`.

**Comando en el VPS:**
```bash
curl -s http://localhost:9080/targets | python3 -m json.tool | grep -E '"job"|"health"|"state"'
```

**Qué debe aparecer:** Jobs `cowrie`, `fail2ban`, `auth` con `"health": "up"` o `"state": "active"`.

> Alternativa si no hay python3: `curl -s http://localhost:9080/targets` y hacer screenshot del JSON crudo.

---

## EV-16 — `ev16_loki_query_cowrie.png`

**Qué demuestra:** Los logs de Cowrie llegan a Loki y son consultables.
...
**Qué debe aparecer:** JSON con array `"values"` conteniendo líneas de log de cowrie con timestamps.

---

## EV-17 — `ev17_loki_query_fail2ban.png`

**Qué demuestra:** Los logs de Fail2Ban llegan a Loki y son consultables.
...
**Qué debe aparecer:** JSON con entradas de fail2ban (bans, found, etc.) con timestamps.
