...
*   🧱 **Red y acceso:** UFW abre SSH en 2929/tcp por defecto (configurable al final del deploy); el panel vive en loopback:8888 y se accede con túnel SSH.
...
| SSH | OpenSSH + Fail2Ban | Admin por clave; honeypot por password | 2929/tcp (configurable) |
...
1. Túnel desde tu equipo (ajusta el puerto si elegiste otro en el asistente):
   ```bash
   ssh -p 2929 -L 8888:127.0.0.1:8888 tu_usuario@tu_vps_ip
   ```
...
- **Proceso recomendado:** VPS limpio Debian/Ubuntu, ejecutar `deploy.sh` como root y usar una clave pública segura (ed25519 o RSA 4096). El asistente permite cambiar el puerto SSH (por defecto 2929) y actualiza UFW; el usuario admin queda con `PasswordAuthentication no`; el honeypot conserva password para telemetría de ataques.
